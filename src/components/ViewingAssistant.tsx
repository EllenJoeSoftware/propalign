...
export default function ViewingAssistant({
  ...
  /**
   * Build a phone-friendly Markdown document from the prep data and trigger
   * a download. Markdown opens cleanly in every text editor / Notes app,
   * renders prettily in any markdown viewer, and is printable as-is.
   */
  const downloadPrep = () => {
    if (!data) return;
    const lines: string[] = [];
    lines.push(`# Viewing Prep — ${propertyTitle}`);
    lines.push('');
    if (data.intro) {
    ...
    if (data.redFlags?.length) {
      lines.push(`## Red flags from the listing`);
      lines.push('');
      for (const rf of data.redFlags) lines.push(`- ${rf}`);
      lines.push('');
    }

    lines.push('---');
    lines.push(`_Generated by PropAlign · ${new Date().toLocaleString()}_`);

    const md = lines.join('\n');
    const blob = new Blob([md], { type: 'text/markdown;charset=utf-8' });
    const url = URL.createObjectURL(blob);

    // Sanitise the title so we don't write `:` `/` etc into the filename.
    const safe = propertyTitle
      .replace(/[^\p{L}\p{N}\s-]/gu, '')
      .trim()
      .replace(/\s+/g, '-')
      .slice(0, 80) || 'viewing-prep';

    const a = document.createElement('a');
    a.href = url;
    a.download = `viewing-prep-${safe}.md`;
    document.body.appendChild(a);
    a.click();
    a.remove();
    // Release the blob URL on next tick — Safari sometimes cancels the
    // download if you revoke synchronously.
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            <footer className="shrink-0 border-t border-[var(--line)] px-5 py-3 flex items-center justify-end gap-2">
              <button
                type="button"
                onClick={downloadPrep}
                disabled={!data || loading}
                className="focus-ring inline-flex items-center gap-1.5 h-8 px-3 rounded-[6px] border border-[var(--line)] bg-[var(--paper-2)] hover:border-[var(--line-strong)] disabled:opacity-40 disabled:cursor-not-allowed text-[11px] font-medium text-[var(--ink-2)] transition-colors"
                title="Download as Markdown — open it on your phone or print it for the viewing"
              >
